...
?>
			...
			foreach ($this->data['idplist'] AS $idpentry) {
				...
				echo '>'.htmlspecialchars($this->t('idpname_' . $idpentry['entityid'])).((isset($this->data['displayLoas']) && !empty($idpentry['loa']))? '  (Loa: ' . $idpentry['loa'] . ')': '').'</option>';
			...
			if($this->data['rememberenabled']) {
				echo('<br/><br/><input type="checkbox" name="remember" value="1" /> ' . $this->t('remember'));
			}
		}
		else {
